...
export default async function mempalacePlugin(input, options) {
    ...
    return {
        event: async ({ event }) => {
            if (event.type === 'session.idle' ||
                ...
                if (sessionID && stateManager.hasPendingMessages(sessionID)) {
                    if (!stateManager.acquireMiningLock(sessionID))
                        return;
        ...
        'experimental.session.compacting': async (_, output) => {
            const state = await ensureInitialized();
            if (state === 'empty') {
                output.context.push('[MemPalace]: This environment has no memory yet. Please proceed with standard logic.');
                return;
            }
            if (state === 'initializing') {
                output.context.push('[MemPalace]: The memory system is being built asynchronously in the background. The current response will not include historical memory context.');
                return;
            }
            const memory = await wakeUp(wing);
            if (memory) {
                const truncatedMemory = memory.length > MAX_MEMORY_LENGTH
        ...
        'experimental.chat.system.transform': async (_, output) => {
            const state = await ensureInitialized();
            if (state === 'empty') {
                output.system.push('[MemPalace]: This environment has no memory yet. Please proceed with standard logic.');
                return;
            }
            if (state === 'initializing') {
                output.system.push('[MemPalace]: The memory system is being built asynchronously in the background. The current response will not include historical memory context.');
                return;
            }
            const memory = await wakeUp(wing);
            if (memory) {
